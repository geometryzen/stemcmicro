...
import { add, exp, guess, is_add, is_multiply, is_num_and_eq_one_half, is_num_and_eq_rational, multiply, num_to_number } from "@stemcmicro/helpers";
...
function integral_of_sum(F: U, X: U, $: ExprContext): U {
    ...
    if (is_cons(F)) {
        result = F.tail().reduce((acc: U, b: U) => add($, acc, integral(b, X, $)), result);
    }
    return result;
}
