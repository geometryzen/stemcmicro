import { create_int, half, is_imu, is_num, is_rat, one, Sym, third, three, two, zero } from "@stemcmicro/atoms";
import { ExprContext } from "@stemcmicro/context";
import { Directive } from "@stemcmicro/directive";
import { complexity } from "@stemcmicro/eigenmath";
import { add, divide, inverse, iszero, is_add, is_inner_or_dot, is_multiply, is_num_and_eq_rational, is_num_and_negative, is_power, multiply, negate, power } from "@stemcmicro/helpers";
...
function simplify_terms(p1: U, $: ExprContext): U {
    ...
    let p2: U = cdr(p1);
    if (is_cons(p2)) {
        p2 = [...p2].reduce((acc: U, p: U) => simplify_rational_expressions(add($, acc, simplify(p, $)), $), zero);
    }
...
function _nestedPowerSymbol(p1: Cons2<Sym, U, U>, $: ExprContext): [U, TFLAGS] {
    ...
    const possibleSolutions: U[] = r.filterElements((sol) => !sol.contains(POWER));
