...
function promote_blades_to_tree<T extends U, K extends U>(maws: MaskAndWeight<T>[], algebra: Algebra<T, K>, metric: METRIC<T>, labels: string[]): K {
    ...
    if (maws.length > 0) {
        const terms = maws.map(function (maw) {
            return {
                blade: create_blade(maw.bitmap, algebra, metric, labels),
                weight: maw.weight
            };
        });
...
export function create_algebra<T extends U, K extends U>(metric: T | T[] | Metric<T>, field: Adapter<T, K>, labels: string[]): Algebra<T, K> {
