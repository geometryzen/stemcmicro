...
xdescribe("py_parse", () => {
    ...
    it("Flt", () => {
        const { trees, errors } = py_parse("0.5");
        expect(errors.length).toBe(0);
        expect(trees.length).toBe(1);
        const tree = trees[0];
...
xdescribe("typhon", function () {
    ...
    it("12345678901234567890", function () {
        const lines: string[] = [`12345678901234567890`];
        const sourceText = lines.join("\n");
        const context = create_script_context({
            syntaxKind: SyntaxKind.PythonScript
        });
    ...
    it("Geometric Algebra I", function () {
        const lines: string[] = [`G30=algebra([1,1,1],["i","j","k"])`, `G30`];
        const sourceText = lines.join("\n");
        const context = create_script_context({
            syntaxKind: SyntaxKind.PythonScript
        });
    ...
    it("Geometric Algebra II", function () {
        const lines: string[] = [`G30=algebra([1,1,1],["i","j","k"])`, `e1=G30[1]`, `e2=G30[2]`, `e3=G30[3]`, `e1`, `e2`, `e3`];
        const sourceText = lines.join("\n");
        const context = create_script_context({
            syntaxKind: SyntaxKind.PythonScript
        });
    ...
    it("i*i", function () {
        const lines: string[] = [`i=sqrt(-1)`, `i*i`];
        const sourceText = lines.join("\n");
        const context = create_script_context({
            syntaxKind: SyntaxKind.PythonScript
        });
