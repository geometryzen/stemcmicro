import { Boo, create_flt, create_int, create_sym, create_sym_ns, create_tensor, Flt, Keyword, Map, Num, Str, Sym } from "@stemcmicro/atoms";
import { assert_cons_or_nil, cons, nil, U } from "@stemcmicro/tree";
...
class Parser {
    ...
    constructor(
        sourceText: string,
        private readonly options: ClojureScriptParseOptions | undefined
    ) {
    ...
    #consumeObject(): U {
        ...
        if (r === endOfString) throw new Error("Readable object not found after S expression comment.");
    ...
    #consumeAtom(): Boo | Sym | Flt | Keyword | Str | Pair | CommentMarker | EOS {
        ...
        const s: false | "quote" | "quasiquote" | "unquote" | "unquote-splicing" = t == "'" ? "quote" : t == "`" ? "quasiquote" : t == "," ? "unquote" : t == ",@" ? "unquote-splicing" : false;
    ...
    #consumeObjectsInSexpComment(err_msg: string): void {
        while (this.#tokens[this.#tokenIdx] == "#;") {
            this.#tokenIdx++;
            if (this.#consumeObject() == endOfString || this.#tokenIdx >= this.#tokens.length) throw new Error(err_msg);
        }
    }
}
