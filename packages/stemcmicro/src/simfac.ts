...
function yysimfac(stack: U[], _: ExprContext): boolean {
    for (let i = 0; i < stack.length; i++) {
        ...
        for (let j = 0; j < stack.length; j++) {
            ...
            //  1 / ((n + 1) n!)  ->  1 / (n + 1)!
            if (is_power(p1) && is_num_and_eq_minus_one(caddr(p1)) && is_power(p2) && is_num_and_eq_minus_one(caddr(p2)) && caadr(p2).equals(FACTORIAL)) {
            ...
            //  n! / (n + 1)!  ->  1 / (n + 1)
            if (is_factorial(p1) && is_power(p2) && is_num_and_eq_minus_one(caddr(p2)) && caadr(p2).equals(FACTORIAL)) {
                const p3 = subtract(_, cadr(p1), cadr(cadr(p2)));
                if (isone(p3, _)) {
                    stack[i] = cadr(p1);
                    stack[j] = one;
