...
describe("G20", function () {
    it("i=e1^e2 squares to -1", function () {
        const lines: string[] = [`G20=algebra([1,1],["ex","ey"])`, `e1=G20[1]`, `e2=G20[2]`, `i=e1^e2`, `i*i`];
    ...
    it("Because i squares to -1, exp(i*theta) has an Euler expansion", function () {
        const lines: string[] = [`G20=algebra([1,1],["ex","ey"])`, `e1=G20[1]`, `e2=G20[2]`, `i=e1^e2`, `exp(i*theta)`];
        const engine = create_script_context({
    ...
    it("Assigning labels to only the basis vectors", function () {
        const lines: string[] = [`G20=algebra([1,1],["East","North"])`, `e1=G20[1]`, `e2=G20[2]`, `e1`, `e2`, `e1|e1`, `e1^e2`];
    ...
    it("Assigning labels to all elements of the algebra", function () {
        const lines: string[] = [`G20=algebra([1,1],["One","East","North","Spin"])`, `e1=G20[1]`, `e2=G20[2]`, `e1`, `e2`, `e1|e1`, `e1^e2`];
        const context = create_script_context({});
        const { values } = context.executeScript(lines.join("\n"));
        assert.strictEqual(context.renderAsInfix(values[0]), "East");
        assert.strictEqual(context.renderAsInfix(values[1]), "North");
    ...
    it("Using G20 as the representation of the Complex Plane", function () {
        const lines: string[] = [`G20=algebra([1,1],["unused","e1","e2","i"])`, `i=G20[1]^G20[2]`, `i`];
        const context = create_script_context({});
        const { values } = context.executeScript(lines.join("\n"));
