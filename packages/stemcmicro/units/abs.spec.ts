...
describe("abs", function () {
    ...
    xit("abs(a+b+i*c)", function () {
        const lines: string[] = [`abs(a+b+i*c)`];
        const engine = create_script_context({
    ...
    xit("x * i", function () {
        const lines: string[] = [`prettyfmt=0`, `i=sqrt(-1)`, `x * i`];
        const engine = create_script_context({});
        const value = assert_one_value_execute(lines.join("\n"), engine);
    ...
    xit("-i * i * x * x", function () {
        const lines: string[] = [`prettyfmt=0`, `i=sqrt(-1)`, `-i * i * x * x`];
        const engine = create_script_context({});
        const value = assert_one_value_execute(lines.join("\n"), engine);
    ...
    xit("(x-i*y)*(x+i*y)", function () {
        const lines: string[] = [`(x-i*y)*(x+i*y)`];
        const engine = create_script_context({
            prolog: stemc_prolog
        });
        const value = assert_one_value_execute(lines.join("\n"), engine);
    ...
    xit("abs(1+2.0*i)", function () {
        // FIXME
        const lines: string[] = [`i=sqrt(-1)`, `abs(1+2.0*i)`];
        const engine = create_script_context({});
        const value = assert_one_value_execute(lines.join("\n"), engine);
    ...
    xit("abs(x)*abs(x)", function () {
        const lines: string[] = [`abs(x)*abs(x)`];
        const engine = create_script_context({
            useCaretForExponentiation: false
        });
        const { values } = engine.executeScript(lines.join("\n"));
    ...
    xit("abs(a+b+c*i)", function () {
        const lines: string[] = [`abs(a+b+c*i)`];
        const engine = create_script_context({
            enable: [Directive.expandPowSum],
            prolog: stemc_prolog,
            useCaretForExponentiation: false
    ...
    xit("abs((1/3)^(1/2))", function () {
        const lines: string[] = [`abs((1/3)^(1/2))`];
        const engine = create_script_context({
    ...
    xit("1+1.0*(-1)^(1/2)", function () {
        const lines: string[] = [`1+1.0*(-1)^(1/2)`];
        const engine = create_script_context({
            useCaretForExponentiation: true
        });
        const actual = assert_one_value_execute(lines.join("\n"), engine);
    ...
    xit("abs(1+1.0*(-1)^(1/2))", function () {
        const lines: string[] = [`abs(1+1.0*(-1)^(1/2))`];
        const engine = create_script_context({
            useCaretForExponentiation: true
        });
        const actual = assert_one_value_execute(lines.join("\n"), engine);
    ...
    xit("abs(exp(i*x))", function () {
        const lines: string[] = [`i=sqrt(-1)`, `abs(exp(i*x))`];
    ...
    xit("abs(exp(a+i*b))", function () {
        const lines: string[] = [`i=sqrt(-1)`, `abs(exp(a+i*b))`];
        const sourceText = lines.join("\n");
