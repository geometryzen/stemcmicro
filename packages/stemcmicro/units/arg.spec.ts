...
xdescribe("arg", function () {
    ...
    it("arg(1)", function () {
        const lines: string[] = [`i=sqrt(-1)`, `arg(1)`];
        const sourceText = lines.join("\n");
    ...
    it("arg(i)", function () {
        const lines: string[] = [`i=sqrt(-1)`, `arg(i)`];
        const sourceText = lines.join("\n");
    ...
    it("arg(-1+0.1*i)", function () {
        const lines: string[] = [`i=sqrt(-1)`, `arg(-1+0.1*i)`];
        const sourceText = lines.join("\n");
    ...
    it("arg(1+i)", function () {
        const lines: string[] = [`i=sqrt(-1)`, `arg(1+i)`];
        const sourceText = lines.join("\n");
...
xdescribe("arg", function () {
    ...
    it("arg(x+i*y)", function () {
        const lines: string[] = [`i=sqrt(-1)`, `arg(x+i*y)`];
        const engine = create_script_context({
            useCaretForExponentiation: true
        });
        const actual = assert_one_value_execute(lines.join("\n"), engine);
    ...
    xit("arg((a+i*b)/(c+i*d))", function () {
        const lines: string[] = [`i=sqrt(-1)`, `arg((a+i*b)/(c+i*d))`];
        const engine = create_script_context({
            useCaretForExponentiation: true
        });
        const actual = assert_one_value_execute(lines.join("\n"), engine);
    ...
    xit("arg(exp(i*pi/3))", function () {
        const lines: string[] = [`arg(exp(i*pi/3))`];
        const engine = create_script_context({
            prolog: stemc_prolog
        });
    ...
    it("arg((-1)**(1/3))", function () {
        const lines: string[] = [`i=sqrt(-1)`, `pi=tau(1/2)`, `arg((-1)**(1/3))`];
        const engine = create_script_context({
            dependencies: ["Imu"]
        });
        const value = assert_one_value_execute(lines.join("\n"), engine);
    ...
    xit("float(arg((-1)**(1/3)))", function () {
        const lines: string[] = [`i=sqrt(-1)`, `pi=tau(1/2)`, `float(arg((-1)**(1/3)))`];
        const engine = create_script_context({
            dependencies: ["Imu"]
        });
        const value = assert_one_value_execute(lines.join("\n"), engine);
    ...
    it("float(1/3*pi)", function () {
        const lines: string[] = [`i=sqrt(-1)`, `pi=tau(1/2)`, `float(1/3*pi)`];
        const engine = create_script_context({
            dependencies: ["Imu"]
        });
        const value = assert_one_value_execute(lines.join("\n"), engine);
    ...
    it("arg(1+exp(i*pi/3))", function () {
        const lines: string[] = [`i=sqrt(-1)`, `pi=tau(1/2)`, `arg(1+exp(i*pi/3))`];
        const engine = create_script_context({
            dependencies: ["Imu"]
        });
        const value = assert_one_value_execute(lines.join("\n"), engine);
    ...
    xit("arg(abs(a)*exp(b+i*pi/5))", function () {
        const lines: string[] = [`i=sqrt(-1)`, `pi=tau(1/2)`, `arg(abs(a)*exp(b+i*pi/5))`];
        const engine = create_script_context({
            assumes: {
                a: { real: false },
                b: { real: true }
    ...
    it("arg((-1)^(1/6)*exp(i*pi/6))", function () {
        const lines: string[] = [`autofactor=0`, `i=sqrt(-1)`, `pi=tau(1)/2`, `arg((-1)^(1/6)*exp(i*pi/6))`];
        const sourceText = lines.join("\n");
