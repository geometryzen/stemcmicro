...
xdescribe("clock", function () {
    it("clock((-1)**(1/2))", function () {
        const lines: string[] = [`i=sqrt(-1)`, `clock((-1)**(1/2))`];
    ...
    it("(1/2+(1/2*3**(1/2))*i)-(1/2+(-1/2*3**(1/2))*i)", function () {
        const lines: string[] = [`clock((1/2+(1/2*3**(1/2))*i)-(1/2+(-1/2*3**(1/2))*i))`];
        const engine = create_script_context({
            dependencies: ["Imu"],
            prolog: stemc_prolog
        });
    ...
    it("clock(exp(i*pi/3))", function () {
        const lines: string[] = [`clock(exp(i*pi/3))`];
        const engine = create_script_context({
            dependencies: ["Imu"],
            prolog: stemc_prolog
        });
    ...
    it("clock(re(log(i)))", function () {
        const lines: string[] = [`i=sqrt(-1)`, `clock(re(log(clock(i))))`];
        const engine = create_script_context({
