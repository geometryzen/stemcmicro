...
import { is_blade, is_boo, is_flt, is_keyword, is_map, is_rat, is_str, is_sym, is_tensor } from "@stemcmicro/atoms";
import { is_cons, is_nil, U } from "@stemcmicro/tree";
...
describe("ClojureScript", function () {
    ...
    it("Sym", function () {
        const lines: string[] = [`foo`, `foo-bar`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine({ allowUndeclaredVars: UndeclaredVars.Nil, syntaxKind: SyntaxKind.ClojureScript });
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("Keyword", function () {
        const lines: string[] = [`:a-keyword`, `::namespaced-keyword`, `:ns/a`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine({ syntaxKind: SyntaxKind.ClojureScript });
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("Evaluation", function () {
        const lines: string[] = [`(mk-sandwich "Bacon" "Lettuce" "Tomato")`, `(def x 5)`, `(if (even? 10) "Even" "Odd")`, `(+ 10 (* 5 2))`, `(* (+ 10 5) 2)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine({ allowUndeclaredVars: UndeclaredVars.Nil, syntaxKind: SyntaxKind.ClojureScript });
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("Tensors in Eigenmath", function () {
        const lines: string[] = [`["Alice", "Bob", "Carol"]`];
        const sourceText = lines.join("\n");
    ...
    it("algebra", function () {
        const lines: string[] = [`(= G30 (algebra [1 1 1] ["i" "j" "k"]))`, `(= e1 (component G30 1))`, `(= e2 (component G30 2))`, `(= e3 (component G30 3))`, `e1`, `e2`, `e3`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine({ syntaxKind: SyntaxKind.ClojureScript });
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("wedge", function () {
        const lines: string[] = [`(= G30 (algebra [1 1 1] ["e1" "e2" "e3"]))`, `(= e1 (component G30 1))`, `(= e2 (component G30 2))`, `(^ e1 e2)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine({ syntaxKind: SyntaxKind.ClojureScript });
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("vbar", function () {
        const lines: string[] = [`(= G30 (algebra [1 1 1] ["e1" "e2" "e3"]))`, `(= e1 (component G30 1))`, `(= e2 (component G30 2))`, `(| e1 e1)`, `(| e1 e2)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine({ syntaxKind: SyntaxKind.ClojureScript });
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("atom", function () {
        const lines: string[] = [`(deref (atom 9))`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine({ syntaxKind: SyntaxKind.ClojureScript });
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("atom", function () {
        const lines: string[] = [`(def a (atom 3))`, `(reset! a 9)`, `(deref a)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine({ syntaxKind: SyntaxKind.ClojureScript });
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("defn", function () {
        const lines: string[] = [`(defn foo [x] (* 2 x))`, `(foo 7)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine({ syntaxKind: SyntaxKind.ClojureScript });
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("defn", function () {
        const lines: string[] = [`(defn foo "doc-string" [x] (* 2 x))`, `(foo 7)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine({ syntaxKind: SyntaxKind.ClojureScript });
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("fn [x] ...", function () {
        const lines: string[] = [`(def triple (fn [x] (* 3 x)))`, `(triple 7)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine({ syntaxKind: SyntaxKind.ClojureScript });
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("fn [x y] ...", function () {
        const lines: string[] = [`(def area (fn [x y] (* x y)))`, `(area 3 5)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine({ syntaxKind: SyntaxKind.ClojureScript });
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("((fn ...) args)", function () {
        const lines: string[] = [`((fn [x y] (* x y)) 3 5)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine({ syntaxKind: SyntaxKind.ClojureScript });
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("((fn ...) args)", function () {
        const lines: string[] = [`((fn [x y] (* x y)) a b)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine({ allowUndeclaredVars: UndeclaredVars.Nil, syntaxKind: SyntaxKind.ClojureScript });
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    xit("BUG", function () {
        const lines: string[] = [`((fn [x y] (* x y)) (3 5) 7)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine({ syntaxKind: SyntaxKind.ClojureScript });
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("function", function () {
        const lines: string[] = [`(def triple (function (* 3 x) (x)))`, `(triple 7)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine({ syntaxKind: SyntaxKind.ClojureScript });
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("let", function () {
        const lines: string[] = [`(let [x 1 y x] y)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine({ syntaxKind: SyntaxKind.ClojureScript });
        try {
            const { trees, errors } = engine.parse(sourceText, {});
    ...
    it("let", function () {
        const lines: string[] = [`(let [num 23] num)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine({ syntaxKind: SyntaxKind.ClojureScript });
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("let", function () {
        const lines: string[] = [`(let [num 23] (+ 2 num))`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine({ syntaxKind: SyntaxKind.ClojureScript });
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("let", function () {
        const lines: string[] = [`(def cell (atom 3))`, `(let [num 137] (reset! cell num))`, `(deref cell)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine({ syntaxKind: SyntaxKind.ClojureScript });
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("quote", function () {
        const lines: string[] = [`(quote (1 2 3 4 5))`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine({ syntaxKind: SyntaxKind.ClojureScript });
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
