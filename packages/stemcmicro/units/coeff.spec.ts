...
describe("coeff", function () {
    it("coeff(40*x^3+30*x^2+20*x+10,3)", function () {
        const lines: string[] = [`coeff(40*x^3+30*x^2+20*x+10,3)`];
    ...
    it("coeff(40*x^3+30*x^2+20*x+10,2)", function () {
        const lines: string[] = [`coeff(40*x^3+30*x^2+20*x+10,2)`];
        const engine = create_script_context({
            useCaretForExponentiation: true
        });
        const { values } = engine.executeScript(lines.join("\n"));
    ...
    it("coeff(40*x^3+30*x^2+20*x+10,1)", function () {
        const lines: string[] = [`coeff(40*x^3+30*x^2+20*x+10,1)`];
        const engine = create_script_context({
            useCaretForExponentiation: true
        });
        const { values } = engine.executeScript(lines.join("\n"));
    ...
    it("coeff(40*x^3+30*x^2+20*x+10,0)", function () {
        const lines: string[] = [`coeff(40*x^3+30*x^2+20*x+10,0)`];
        const engine = create_script_context({
            useCaretForExponentiation: true
        });
        const { values } = engine.executeScript(lines.join("\n"));
