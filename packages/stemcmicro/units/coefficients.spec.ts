...
describe("coefficients", function () {
    it("coefficients(x,x)", function () {
        const lines: string[] = [`coefficients(x,x)`];
    ...
    it("coefficients(a0,x)", function () {
        const lines: string[] = [`coefficients(a0,x)`];
        const sourceText = lines.join("\n");
        const engine = create_engine();
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("coefficients(a0+a1*x,x)", function () {
        const lines: string[] = [`coefficients(a0+a1*x,x)`];
        const sourceText = lines.join("\n");
        const engine = create_engine();
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("coefficients(a0+a1*x+a2*x**2,x)", function () {
        const lines: string[] = [`coefficients(a0+a1*x+a2*x**2,x)`];
        const sourceText = lines.join("\n");
        const engine = create_engine();
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("coefficients(a0+a1*x+a2*x**2+a3*x**3,x)", function () {
        const lines: string[] = [`coefficients(a0+a1*x+a2*x**2+a3*x**3,x)`];
        const sourceText = lines.join("\n");
        const engine = create_engine();
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("coefficients(a0+a1*x+a2*x**2+a3*x**3+a4*x**4,x)", function () {
        const lines: string[] = [`coefficients(a0+a1*x+a2*x**2+a3*x**3+a4*x**4,x)`];
        const sourceText = lines.join("\n");
        const engine = create_engine();
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("coefficients(a9*x**9,x)", function () {
        const lines: string[] = [`coefficients(a9*x**9,x)`];
        const sourceText = lines.join("\n");
        const engine = create_engine();
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("coefficients(a10*x**10,x)", function () {
        const lines: string[] = [`coefficients(a10*x**10,x)`];
        const sourceText = lines.join("\n");
        const engine = create_engine();
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("coefficients(k*x**11,x)", function () {
        const lines: string[] = [`coefficients(k*x**11,x)`];
        const sourceText = lines.join("\n");
        const engine = create_engine();
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
