...
describe("contract-sandbox", function () {
    it("contract([[1,2],[3,4]],1,2)", function () {
        const lines: string[] = [`A=[[a11,a12],[a21,a22]]`, `B=[[b11,b12],[b21,b22]]`, `contract(outer(A,B),2,3)`];
...
describe("contract", function () {
    ...
    xit("contract(0.0)", function () {
        const lines: string[] = [`contract(0.0)`];
    ...
    it("409741429887649/166966608033225 + 1/39", function () {
        const lines: string[] = [`409741429887649/166966608033225 + 1/39`];
        const engine = create_script_context({ dependencies: ["Flt"] });
        const actual = assert_one_value_execute(lines.join("\n"), engine);
    ...
    it("contract(hilbert(50))", function () {
        const lines: string[] = [`contract(hilbert(50))`];
        const engine = create_script_context({ dependencies: ["Flt"] });
        const actual = assert_one_value_execute(lines.join("\n"), engine);
    ...
    it("contract([[a,b],[c,d]])", function () {
        const lines: string[] = [`contract([[a,b],[c,d]])`];
        const engine = create_script_context({ dependencies: [] });
        const actual = assert_one_value_execute(lines.join("\n"), engine);
    ...
    it("contract([[1,2],[3,4]],1,2)", function () {
        const lines: string[] = [`contract([[1,2],[3,4]],1,2)`];
        const engine = create_script_context({ dependencies: [] });
        const actual = assert_one_value_execute(lines.join("\n"), engine);
