...
describe("coverage", function () {
    it("abs(x)", function () {
        const lines: string[] = [`X=[x,y,z]`, `abs(X)`];
    ...
    it("adj(x)", function () {
        const lines: string[] = [`A=[[a,b],[c,d]]`, `adj(A)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("det(x)", function () {
        const lines: string[] = [`A=[[a,b],[c,d]]`, `det(A)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("inv(x)", function () {
        const lines: string[] = [`A=[[a,b],[c,d]]`, `inv(A)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("det(A) * inv(A)", function () {
        const lines: string[] = [`A=[[a,b],[c,d]]`, `det(A) * inv(A)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("adj(x)", function () {
        const lines: string[] = [`A=[[a,b],[c,d]]`, `adj(A) == det(A) * inv(A)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("and(a,b,...)", function () {
        const lines: string[] = [`and(false,false)`, `and(false,true)`, `and(true,false)`, `and(true,true)`, `and(0,0)`, `and(0,1)`, `and(1,0)`, `and(1,1)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("arccosh(x)", function () {
        const lines: string[] = [`arccosh(1/2)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("arcsinh(x)", function () {
        const lines: string[] = [`arcsinh(1/2)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("Eigenmath arctan(y,x)", function () {
        ...
        assert.strictEqual(values.length, 1);
        assert.strictEqual(strip_whitespace(engine.renderAsString(values[0], { format: "Ascii" })), strip_whitespace("1---pi2"));
        assert.strictEqual(strip_whitespace(engine.renderAsString(values[0], { format: "Human" })), strip_whitespace("1/2pi"));
        assert.strictEqual(strip_whitespace(engine.renderAsString(values[0], { format: "Infix" })), strip_whitespace("1/2*pi"));
        assert.strictEqual(strip_whitespace(engine.renderAsString(values[0], { format: "LaTeX" })), strip_whitespace("\\frac{\\pi}{2}"));
    ...
    it("arctanh(x)", function () {
        const lines: string[] = [`arctanh(1/2)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("arg(z)", function () {
        const lines: string[] = [`i = sqrt(-1)`, `arg(2 - 3 * i)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("besselj(x,n)", function () {
        const lines: string[] = [`besselj(0,0)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("bessely(x,n)", function () {
        const lines: string[] = [`bessely(0,0)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("binding(s)", function () {
        const lines: string[] = [`p = quote((x + 1)^2)`, `binding(p)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("ceiling(x)", function () {
        const lines: string[] = [`ceiling(1/2)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("check(x)", function () {
        const lines: string[] = [`i = sqrt(-1)`, `A = exp(i * pi)`, `A`, `B = -1`, `B`, `check(A==B)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("choose(n,k)", function () {
        const lines: string[] = [`choose(52,5)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("circexp(x)", function () {
        const lines: string[] = [`i=sqrt(-1)`, `circexp(cos(x) + i * sin(x))`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
        ...
        assert.strictEqual(values.length, 1);
        assert.strictEqual(strip_whitespace(engine.renderAsString(values[0], renderConfig)), strip_whitespace("e^(i*x)"));
        engine.release();
    });
    xit("clear", function () {
    ...
    xit("clock(z)", function () {
        const lines: string[] = [`i=sqrt(-1)`, `clock(2 - 3 * i)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("coeff(p,x,n)", function () {
        const lines: string[] = [`coeff(a*x^3+b*x^2+c*x+d,x,3)`, `coeff(a*x^3+b*x^2+c*x+d,x,2)`, `coeff(a*x^3+b*x^2+c*x+d,x,1)`, `coeff(a*x^3+b*x^2+c*x+d,x,0)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("coefficients(p)", function () {
        const lines: string[] = [`coefficients(a*x^3+b*x^2+c*x+d,x)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("cofactor(m,i,j)", function () {
        const lines: string[] = [`A=[[a,b],[c,d]]`, `cofactor(A,1,2) == adj(A)[2,1]`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("conj(z)", function () {
        const lines: string[] = [`i=sqrt(-1)`, `conj(2 - 3 * i)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("contract(a,i,j)", function () {
        const lines: string[] = [`A=[[a,b],[c,d]]`, `contract(A)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("cosh(x)", function () {
        const lines: string[] = [`circexp(cosh(x))`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("d(f,x,...)", function () {
        const lines: string[] = [`d(sin(x),x)`, `d(sin(x),x,x)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("defint(f,x,a,b)", function () {
        const lines: string[] = [`f = (1 + cos(theta)^2) * sin(theta)`, `defint(f, theta, 0, pi, phi, 0, 2 * pi)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("deg(p,x)", function () {
        const lines: string[] = [`deg(a*x^2+b*x+c,x)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("denominator(x)", function () {
        const lines: string[] = [`denominator(a/b)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("dim(a,m)", function () {
        const lines: string[] = [`A=[[1,2],[3,4],[5,6]]`, `dim(A,1)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("dot(a,b,...)", function () {
        const lines: string[] = [`A=[[1,2],[3,4]]`, `B=[5,6]`, `X=dot(inv(A),B)`, `X`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("eigenval(m)", function () {
        const lines: string[] = [`A=((1,2),(3,4))`, `eigenval(A)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("eigenvec(m)", function () {
        const lines: string[] = [`A=((1,2),(3,4))`, `A=A+transpose(A)`, `Q=eigenvec(A)`, `D=dot(transpose(Q),A,Q)`, `dot(Q,D,tanspose(Q))`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("eval(f,x,a,y,b,...)", function () {
        const lines: string[] = [`f=sqrt(x^2 + y^2)`, `eval(f,x,3,y,4)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("exp(x)", function () {
        const lines: string[] = [`pi=tau(1/2)`, `i=sqrt(-1)`, `exp(i * pi)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("expcos(z)", function () {
        const lines: string[] = [`i=sqrt(-1)`, `expcos(z)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
        ...
        assert.strictEqual(values.length, 1);
        assert.strictEqual(strip_whitespace(engine.renderAsString(values[0], renderConfig)), strip_whitespace("1/2*e^(-i*z)+1/2*e^(i*z)"));
        engine.release();
    });
    xit("expcosh(z)", function () {
        const lines: string[] = [`i=sqrt(-1)`, `expcosh(z)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("expsin(z)", function () {
        const lines: string[] = [`i=sqrt(-1)`, `expsin(z)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
        ...
        assert.strictEqual(values.length, 1);
        assert.strictEqual(strip_whitespace(engine.renderAsString(values[0], renderConfig)), strip_whitespace("1/2*i*e^(-i*z)-1/2*i*e^(i*z)"));
        engine.release();
    });
    xit("expsinh(z)", function () {
        const lines: string[] = [`i=sqrt(-1)`, `expsinh(z)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("exptan(z)", function () {
        const lines: string[] = [`i=sqrt(-1)`, `exptan(z)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("exptanh(z)", function () {
        const lines: string[] = [`i=sqrt(-1)`, `exptanh(z)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("factorial(n)", function () {
        const lines: string[] = [`factorial(20)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("float(x)", function () {
        const lines: string[] = [`float(212^17)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("for(i,j,k,a,b,...)", function () {
        const lines: string[] = [`for(k,1,3,A=k,print(A))`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("hadamard(a,b,...)", function () {
        const lines: string[] = [`A=((a11,a12),(a21,a22))`, `B=((b11,b12),(b21,b22))`, `hadamard(A,B)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("hermite(x,n)", function () {
        const lines: string[] = [`hermite(x,0)`, `hermite(x,1)`, `hermite(x,2)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("hilbert(n)", function () {
        const lines: string[] = [`hilbert(0)`, `hilbert(1)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("imag(z)", function () {
        const lines: string[] = [`i=sqrt(-1)`, `imag(2 - 3 * i)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("infix(x)", function () {
        const lines: string[] = [`p=(x+1)^2`, `infix(p)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("inner(a,b,...)", function () {
        const lines: string[] = [`A=[[a,b],[c,d]]`, `B=[x,y]`, `inner(A,B)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("integral(f,x)", function () {
        const lines: string[] = [`integral(x^2,x)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("inv(m)", function () {
        const lines: string[] = [`A=[[1,2],[3,4]]`, `inv(A)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("isprime(n)", function () {
        const lines: string[] = [`isprime(0)`, `isprime(1)`, `isprime(2)`, `isprime(3)`, `isprime(4)`, `isprime(5)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("j", function () {
        const lines: string[] = [`j=sqrt(-1)`, `1/sqrt(-1)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("laguerre(x,n,a)", function () {
        const lines: string[] = [`laguerre(x,0)`, `laguerre(x,1)`, `laguerre(x,2)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("last", function () {
        const lines: string[] = [`212^17`, `last^(1/17)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("leading(p,x)", function () {
        const lines: string[] = [`leading(5*x^2+x+1,x)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("legendre(x,n,m)", function () {
        const lines: string[] = [`legendre(x,0)`, `legendre(x,1)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("lookup(x)", function () {
        const lines: string[] = [`x=quote(1+2)`, `lookup(x)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("mag(z)", function () {
        const lines: string[] = [`i=sqrt(-1)`, `mag(x + i * y)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("minor(m,i,j)", function () {
        const lines: string[] = [`A=((1,2,3),(4,5,6),(7,8,9))`, `minor(A,1,1)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("minormatrix(m,i,j)", function () {
        const lines: string[] = [`A=((1,2,3),(4,5,6),(7,8,9))`, `minormatrix(A,1,1)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("noexpand(x)", function () {
        const lines: string[] = [`noexpand((x+1)^2/(x+1))`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("nroots(p,x)", function () {
        const lines: string[] = [`p=x^5 - 1`, `nroots(p,x)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("numerator(x)", function () {
        const lines: string[] = [`numerator(a/b)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("outer(a,b,...)", function () {
        const lines: string[] = [`A=[a,b,c]`, `B=[x,y,z]`, `outer(A,B)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("polar(z)", function () {
        const lines: string[] = [`i=sqrt(-1)`, `polar(x - i * y)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("prime(n)", function () {
        const lines: string[] = [`prime(1)`, `prime(2)`, `prime(3)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("print(a,b,...)", function () {
        const lines: string[] = [`print(1,2,3)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("print2dascii(a,b,...)", function () {
        const lines: string[] = [`print2dascii(1,2,3)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("printcomputer(a,b,...)", function () {
        const lines: string[] = [`printcomputer(1,2,3)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("printlatex(a,b,...)", function () {
        const lines: string[] = [`printlatex(1,2,3)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("printlist(a,b,...)", function () {
        const lines: string[] = [`printlist(1,2,3)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("printhuman(a,b,...)", function () {
        const lines: string[] = [`printhuman(1,2,3)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("product(i,j,k,f)", function () {
        const lines: string[] = [`product(j,1,3,x + j)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("product(y)", function () {
        const lines: string[] = [`y=(1,2,3,4)`, `product(y)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("quote(x)", function () {
        const lines: string[] = [`quote((x + 1)^2)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("quotient(p,q,x)", function () {
        const lines: string[] = [`quotient(x^2+1,x+1)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("rank(a)", function () {
        const lines: string[] = [`A=[[a,b],[c,d]]`, `rank(A)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("real(z)", function () {
        const lines: string[] = [`i=sqrt(-1)`, `real(2 - 3 * i)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("rect(z)", function () {
        const lines: string[] = [`i=sqrt(-1)`, `rect(exp(i*x))`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("roots(p,x)", function () {
        const lines: string[] = [`p=(x+1)*(x-2)`, `roots(p,x)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("rotate(u,s,k,...)", function () {
        const lines: string[] = [`psi=(1,0,0,0)`, `rotate(psi,H,0)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("run(x)", function () {
        const lines: string[] = [`run("foo.txt")`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("shape(x)", function () {
        const lines: string[] = [`shape([a,b,c])`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("simplify(x)", function () {
        const lines: string[] = [`simplify(cos(x)^2+sin(x)^2)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("sqrt(x)", function () {
        const lines: string[] = [`sqrt(factorial(10))`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
        ...
        assert.strictEqual(values.length, 1);
        assert.strictEqual(strip_whitespace(engine.renderAsString(values[0], renderConfig)), strip_whitespace("7^(1/2)*720"));
        engine.release();
    });
    xit("stop", function () {
    ...
    it("subst(a,b,c)", function () {
        const lines: string[] = [`subst(a,b,c)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("sum(i,j,k,f)", function () {
        const lines: string[] = [`sum(j,1,5,x^j)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("tan(x)", function () {
        const lines: string[] = [`pi=tau(1/2)`, `tan(pi/4)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("tanh(x)", function () {
        const lines: string[] = [`circexp(tanh(x))`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
        ...
        assert.strictEqual(values.length, 1);
        assert.strictEqual(strip_whitespace(engine.renderAsString(values[0], renderConfig)), strip_whitespace("e^(2*x)/(1+e^(2*x))-1/(1+e^(2*x))"));
        engine.release();
    });
    xit("taylor(f,x,n,a)", function () {
        const lines: string[] = [`taylor(1/(1-x),x,5)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("test(a,b,c,d,...)", function () {
        const lines: string[] = [`A=1`, `B=1`, `test(A=B, "yes", "no")`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("transpose(a,i,j)", function () {
        const lines: string[] = [`A=[[a,b],[c,d]]`, `transpose(A)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("tty", function () {
        const lines: string[] = [`tty=1`, `(x+1)^2`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("zero(i,j,...)", function () {
        const lines: string[] = [`A=zero(3,3)`, `for(k,1,3,A[k,k]=k)`, `A`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineConfig);
        const { trees, errors } = engine.parse(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
