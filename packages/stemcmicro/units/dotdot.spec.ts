...
import { create_sym, is_boo, is_flt, is_str, JsObject } from "@stemcmicro/atoms";
...
describe("dotdot", function () {
    it("svalue", function () {
        const lines: string[] = [`(.. e -target -svalue)`];
    ...
    it("nvalue", function () {
        const lines: string[] = [`(.. e -target -nvalue)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine({ syntaxKind: SyntaxKind.ClojureScript });
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("bvalue", function () {
        const lines: string[] = [`(.. e -target -bvalue)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine({ syntaxKind: SyntaxKind.ClojureScript });
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
