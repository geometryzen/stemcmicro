...
describe("edge", function () {
    ...
    it("^", function () {
        const lines: string[] = [`G30=algebra([1,1,1],["e1","e2","e3"])`, `e1=G30[1]`, `e2=G30[2]`, `e3=G30[3]`, `e1^e1`, `e1^e2`, `e1^e3`, `e2^e1`, `e2^e2`, `e2^e3`, `e3^e1`, `e3^e2`, `e3^e3`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineOptions);
        const { module, errors } = engine.parseModule(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("==", function () {
        const lines: string[] = [`0 == 0`, `0 == 1`, `1 == 0`, `1 == 1`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineOptions);
        const { module, errors } = engine.parseModule(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("A[i,j,...]=", function () {
        const lines: string[] = [`A=unit(2)`, `A[1,1]=a`, `A[1,2]=b`, `A[2,1]=c`, `A[2,2]=d`, `A`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineOptions);
        const { module, errors } = engine.parseModule(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("algebra(metric,labels)", function () {
        const lines: string[] = [`G30=algebra([1,1,1],["e1","e2","e3"])`, `G30`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineOptions);
        const { module, errors } = engine.parseModule(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("and(a,b,...)", function () {
        const lines: string[] = [`and(0,0)`, `and(0,1)`, `and(1,0)`, `and(1,1)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineOptions);
        const { module, errors } = engine.parseModule(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    xit("arg(z)", function () {
        const lines: string[] = [`i = sqrt(-1)`, `arg(1)`, `arg(i)`, `arg(-1)`, `arg(-i)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineOptions);
        const { module, errors } = engine.parseModule(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("hermite(x,n)", function () {
        const lines: string[] = [`hermite(x,0)`, `hermite(x,1)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineOptions);
        const { module, errors } = engine.parseModule(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("laguerre(x,n,a)", function () {
        const lines: string[] = [`laguerre(x,0)`, `laguerre(x,1)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineOptions);
        const { module, errors } = engine.parseModule(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("not(x)", function () {
        const lines: string[] = [`not(0)`, `not(1)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineOptions);
        const { module, errors } = engine.parseModule(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("or(a,b,...)", function () {
        const lines: string[] = [`or(0,0)`, `or(0,1)`, `or(1,0)`, `or(1,1)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineOptions);
        const { module, errors } = engine.parseModule(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("sin(x)", function () {
        const lines: string[] = [`pi=tau(1/2)`, `sin(pi/2)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineOptions);
        const { module, errors } = engine.parseModule(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("tau(n)", function () {
        const lines: string[] = [`tau(0)`, `tau(1/2)`, `tau(1)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineOptions);
        const { module, errors } = engine.parseModule(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("uom(s)", function () {
        const lines: string[] = [`uom("kilogram")`, `uom("meter")`, `uom("second")`, `uom("coulomb")`, `uom("ampere")`, `uom("kelvin")`, `uom("mole")`, `uom("candela")`, `uom("newton")`, `uom("joule")`, `uom("watt")`, `uom("volt")`, `uom("tesla")`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineOptions);
        const { module, errors } = engine.parseModule(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
    ...
    it("xyz(a,b,c,...)", function () {
        const lines: string[] = [`A=xyz(1,2,3)`, `A`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine(engineOptions);
        const { module, errors } = engine.parseModule(sourceText, parseConfig);
        assert.strictEqual(errors.length, 0);
