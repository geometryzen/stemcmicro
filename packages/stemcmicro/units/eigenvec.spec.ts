...
describe("eigenvec", function () {
    it("A", function () {
        const lines: string[] = [`A=[[1,2,3],[2,6,4],[3,4,5]]`, `Q=eigenvec(A)`, `Q`];
    ...
    it("B", function () {
        const lines: string[] = [`A=[[1,2,3],[2,6,4],[3,4,5]]`, `Q=eigenvec(A)`, `transpose(Q)`];
        const engine = create_script_context({});
        const { values } = engine.executeScript(lines.join("\n"));
    ...
    it("C", function () {
        const lines: string[] = [`A=[[1,2,3],[2,6,4],[3,4,5]]`, `Q=eigenvec(A)`, `D=dot(transpose(Q),A,Q)`, `dot(Q,D,transpose(Q))`];
        const engine = create_script_context({});
        const { values } = engine.executeScript(lines.join("\n"));
