...
describe("float", function () {
    it("A", function () {
        const lines: string[] = [`float(tau(1/2))`];
    ...
    xit("E", function () {
        const lines: string[] = [`float((1+2*i)^(1/2))`];
        const engine = create_script_context({
            dependencies: ["Flt"],
    ...
    it("G", function () {
        const lines: string[] = [`pi=tau(1/2)`, `float(pi)`];
        const engine = create_script_context({
            dependencies: ["Flt"]
        });
        const actual = assert_one_value_execute(lines.join("\n"), engine);
    ...
    it("float(exp(1))", function () {
        const lines: string[] = [`float(exp(1))`];
    ...
    it("float(-1)", function () {
        ...
        const svg = [`<svg height='36'width='49'>`, `<text style='font-family:"Times New Roman";font-size:24px;' x='10' y='26'>&minus;</text>`, `<text style='font-family:"Times New Roman";font-size:24px;' x='26.53515625' y='26'>1</text>`, `</svg>`].join(
            ""
        );
