...
describe("imag", function () {
    ...
    it("imag(i)", function () {
        const lines: string[] = [`i=sqrt(-1)`, `imag(i)`];
        const engine = create_script_context({
            dependencies: ["Imu"]
        });
        const { values } = engine.executeScript(lines.join("\n"));
    ...
    it("imag(i*y)", function () {
        const lines: string[] = [`i=sqrt(-1)`, `imag(i*y)`];
        const engine = create_script_context({
            dependencies: ["Imu"]
        });
        const { values } = engine.executeScript(lines.join("\n"));
    ...
    it("imag(a+i*b)", function () {
        const lines: string[] = [`i=sqrt(-1)`, `imag(a+i*b)`];
        const engine = create_script_context({
            dependencies: ["Imu"]
        });
        const { values } = engine.executeScript(lines.join("\n"));
    ...
    xit("imag(x+i*y)", function () {
        const lines: string[] = [`i=sqrt(-1)`, `imag(x+i*y)`];
        const engine = create_script_context({
    ...
    xit("imag(1/(x+i*y))", function () {
        const lines: string[] = [`i=sqrt(-1)`, `imag(1/(x+i*y))`];
        const engine = create_script_context({
            useCaretForExponentiation: true
        });
        const actual = assert_one_value_execute(lines.join("\n"), engine);
    ...
    it("imag((-1)^(1/3))", function () {
        const lines: string[] = [`i=sqrt(-1)`, `pi=tau(1/2)`, `imag((-1)^(1/3))`];
    ...
    it("imag((-1)^(1/6))", function () {
        const lines: string[] = [`autofactor=0`, `i=sqrt(-1)`, `pi=tau(1)/2`, `imag((-1)^(1/6))`];
        const sourceText = lines.join("\n");
    ...
    it("imag((-1)^(1/6)*3^(1/2))", function () {
        const lines: string[] = [`autofactor=0`, `i=sqrt(-1)`, `pi=tau(1)/2`, `imag((-1)^(1/6)*3^(1/2))`];
        const sourceText = lines.join("\n");
    ...
    it("imag(1/(a+i*b))", function () {
        const lines: string[] = [`i=sqrt(-1)`, `imag(1/(a+i*b))`];
        const sourceText = lines.join("\n");
    ...
    it("imag(exp(i*x))", function () {
        const lines: string[] = [`i=sqrt(-1)`, `imag(exp(i*x))`];
        const engine = create_script_context({
            dependencies: ["Imu"],
    ...
    it("imag(exp(a*i*x))", function () {
        const lines: string[] = [`i=sqrt(-1)`, `imag(exp(a*i*x))`];
        const engine = create_script_context({
            dependencies: ["Imu"],
            useCaretForExponentiation: false
        });
    ...
    it("imag(exp(i))", function () {
        const lines: string[] = [`i=sqrt(-1)`, `imag(exp(i))`];
        const engine = create_script_context({
            dependencies: ["Imu"],
            useCaretForExponentiation: false
        });
    ...
    it("imag((a**2)**(1/2))", function () {
        const lines: string[] = [`imag((a**2)**(1/2))`];
        const engine = create_script_context({});
