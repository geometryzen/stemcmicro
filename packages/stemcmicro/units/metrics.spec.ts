...
describe("metrics", function () {
    it("Euclidean", function () {
        const lines: string[] = [`G11 = algebra([1.0, 1.0], ["i", "j"])`, `e1 = G11[1]`, `e2 = G11[2]`, `abs(e1)`];
    ...
    it("General", function () {
        const lines: string[] = [`G11 = algebra([g11, g22], ["i", "j"])`, `e1 = G11[1]`, `e2 = G11[2]`, `abs(e1)`, `e1|e1`, `e1|e2`, `e2|e1`, `e2|e2`];
        const engine = create_script_context({
            dependencies: ["Blade"]
        });
        const { values } = engine.executeScript(lines.join("\n"));
