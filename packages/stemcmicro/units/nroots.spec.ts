...
describe("nroots", function () {
    ...
    it("nroots(x**2-1)", function () {
        const lines: string[] = [`nroots(x**2-1)`];
        const engine = create_script_context({
            prolog: stemc_prolog
        });
    ...
    it("nroots((1+i)*x**2+1)", function () {
        const lines: string[] = [`i=sqrt(-1)`, `nroots((1+i)*x**2+1)`];
        const engine = create_script_context({
            prolog: stemc_prolog
        });
        const { values } = engine.executeScript(lines.join("\n"));
    ...
    it("nroots(sqrt(2)*exp(i*pi/4)*x^2+1)", function () {
        const lines: string[] = [`nroots(sqrt(2)*exp(i*pi/4)*x^2+1)`];
        const engine = create_script_context({
            prolog: stemc_prolog,
            useCaretForExponentiation: true
        });
    ...
    it("nroots(x^4+1)", function () {
        const lines: string[] = [`nroots(x^4+1)`];
        const engine = create_script_context({
            useCaretForExponentiation: true
        });
        const { values } = engine.executeScript(lines.join("\n"));
