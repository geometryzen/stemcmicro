...
xdescribe("polar", function () {
    it("polar(i)", function () {
        const lines: string[] = [`i=sqrt(-1)`, `pi=tau(1/2)`, `polar(i)`];
    ...
    it("1+i", function () {
        const lines: string[] = [`i=sqrt(-1)`, `pi=tau(1/2)`, `polar(1+i)`];
        const engine = create_script_context({
            dependencies: ["Imu"]
        });
        const value = assert_one_value_execute(lines.join("\n"), engine);
    ...
    it("polar(3+4*i)", function () {
        const lines: string[] = [`i=sqrt(-1)`, `pi=tau(1/2)`, `polar(3+4*i)`];
        const engine = create_script_context({
            dependencies: ["Imu"]
        });
        const value = assert_one_value_execute(lines.join("\n"), engine);
    ...
    it("polar(x+i*y)", function () {
        const lines: string[] = [`polar(x+i*y)`];
        const engine = create_script_context({
            prolog: stemc_prolog,
