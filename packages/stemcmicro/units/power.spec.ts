...
describe("Exponentiation", function () {
    ...
    it("test A", function () {
        const lines: string[] = [`a**1/2 + a**1/2`];
        const engine = create_script_context();
        const actual = assert_one_value_execute(lines.join("\n"), engine);
    ...
    it("1/(-a*b-x*b^2)+1/(a*b+x*b^2)", function () {
        const lines: string[] = [`1/(-a*b-x*b^2)+1/(a*b+x*b^2)`];
        const engine = create_script_context({
            disable: [Directive.factoring],
            useCaretForExponentiation: true
        });
    ...
    it("1/(b*(a+b*x))+1/(-a*b-x*b^2)", function () {
        const lines: string[] = [`1/(b*(a+b*x))+1/(-a*b-x*b^2)`];
        const engine = create_script_context({
            disable: [Directive.factoring],
            useCaretForExponentiation: true
        });
...
describe("Free Body Diagram", function () {
    ...
    it("B", function () {
        const lines: string[] = [`G20=algebra([1],["e"])`, `e=G20[1]`, `1/e`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine();
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("C", function () {
        const lines: string[] = [`G20=algebra([1],["e"])`, `e=G20[1]`, `1/(1/e)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine();
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
