...
describe("real", function () {
    ...
    it("exp(x*i)", function () {
        const lines: string[] = [`i=sqrt(-1)`, `real(exp(x*i))`];
        const engine = create_script_context({
            dependencies: ["Imu"]
        });
        const { values } = engine.executeScript(lines.join("\n"));
    ...
    it("real(x+i*y) => x, when variables are assumed to be real.", function () {
        const lines: string[] = [`i=sqrt(-1)`, `real(x+i*y)`];
        const engine = create_script_context({
            dependencies: ["Imu"]
        });
        const { values } = engine.executeScript(lines.join("\n"));
    ...
    xit("real(exp(i*x)) => cos(x)", function () {
        const lines: string[] = [`i=sqrt(-1)`, `real(exp(i*x))`];
        const engine = create_script_context({
            dependencies: ["Imu"]
        });
        const { values } = engine.executeScript(lines.join("\n"));
    ...
    xit("", function () {
        const lines: string[] = [`i=sqrt(-1)`, `real(1/(x+i*y))`];
        const engine = create_script_context({
    ...
    it("real(log(i))", function () {
        const lines: string[] = [`i=sqrt(-1)`, `real(log(i))`];
        const engine = create_script_context({
            useCaretForExponentiation: true
        });
        const actual = assert_one_value_execute(lines.join("\n"), engine);
    ...
    it("clock(real(log(i)))", function () {
        const lines: string[] = [`i=sqrt(-1)`, `clock(real(log(clock(i))))`];
        const engine = create_script_context({
            useCaretForExponentiation: true
        });
        const actual = assert_one_value_execute(lines.join("\n"), engine);
    ...
    it("real(i*log(3))", function () {
        const lines: string[] = [`i=sqrt(-1)`, `pi=tau(1/2)`, `real(i*log(3))`];
        const engine = create_script_context({
            dependencies: ["Imu"]
        });
    ...
    it("real((-1)^(1/3))", function () {
        const lines: string[] = [`i=sqrt(-1)`, `pi=tau(1/2)`, `real((-1)^(1/3))`];
    ...
    it("real(exp(a*i*x))", function () {
        const lines: string[] = [`i=sqrt(-1)`, `real(exp(a*i*x))`];
        const engine = create_script_context({
            dependencies: ["Imu"],
            useCaretForExponentiation: false
        });
    ...
    it("real(exp(i))", function () {
        const lines: string[] = [`i=sqrt(-1)`, `real(exp(i))`];
        const engine = create_script_context({
            dependencies: ["Imu"],
            useCaretForExponentiation: false
        });
    ...
    xit("real((a**2)**(1/2))", function () {
        const lines: string[] = [`real((a**2)**(1/2))`];
        const engine = create_script_context({});
        const value = assert_one_value_execute(lines.join("\n"), engine);
