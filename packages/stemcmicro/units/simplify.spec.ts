...
describe("simplify", function () {
    it("A0", function () {
        const lines: string[] = [`i=sqrt(-1)`, `pi=tau(1)/2`, `simplify(1/4*i*pi)`];
    ...
    it("A1", function () {
        const lines: string[] = [`i=sqrt(-1)`, `pi=tau(1)/2`, `simplify(exp(1/4*i*pi))`];
    ...
    it("A2", function () {
        const lines: string[] = [`i=sqrt(-1)`, `pi=tau(1)/2`, `simplify(exp(-3/4*i*pi))`];
        const engine = create_script_context({ useCaretForExponentiation: true });
        const actual = assert_one_value_execute(lines.join("\n"), engine);
        assert.strictEqual(engine.renderAsInfix(actual), "-e^(1/4*i*pi)");
    ...
    it("D", function () {
        const lines: string[] = [`simplify(S*cos(x)**2+S*sin(x)**2)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine();
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("E", function () {
        const lines: string[] = [`simplify(-M+S*cos(x)**2+S*sin(x)**2)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine();
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("F1", function () {
        const lines: string[] = [`simplify(-sin(x))`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine();
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("F2", function () {
        const lines: string[] = [`simplify(-cos(x))`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine();
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("F3", function () {
        const lines: string[] = [`simplify(-M*cos(x))`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine();
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("F4", function () {
        const lines: string[] = [`simplify(-M*cos(x)+S*cos(x)**2+S*sin(x)**2)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine();
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("G", function () {
        const lines: string[] = [`simplify(-M*sin(x)+S*cos(x)**2+S*sin(x)**2)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine();
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("H", function () {
        const lines: string[] = [`simplify(-9.81*M*cos(x)+S*cos(x)**2+S*sin(x)**2)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine();
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("I", function () {
        const lines: string[] = [`simplify(-9.81*cos(x)+S*cos(x)**2+S*sin(x)**2)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine();
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("J", function () {
        const lines: string[] = [`simplify(-9.81*M+S*cos(x)**2+S*sin(x)**2)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine();
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("K", function () {
        const lines: string[] = [`simplify(-K*M*cos(x)+S*cos(x)**2+S*sin(x)**2)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine();
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("M", function () {
        const lines: string[] = [`simplify(-9.81*M*L+S*cos(x)**2+S*sin(x)**2)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine();
        const { trees, errors } = engine.parse(sourceText, { useCaretForExponentiation: false });
        assert.strictEqual(errors.length, 0);
    ...
    it("O", function () {
        const lines: string[] = [`simplify(3*M*cos(x)+S*cos(x)**2+S*sin(x)**2)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine();
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("P", function () {
        const lines: string[] = [`simplify(3.0*M*cos(x)+S*cos(x)**2+S*sin(x)**2)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine();
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("Q", function () {
        const lines: string[] = [`kg=uom("kilogram")`, `simplify(-9.81*M*cos(x)*kg+S*cos(x)**2+S*sin(x)**2)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine();
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
    ...
    it("R", function () {
        const lines: string[] = [`kg=uom("kilogram")`, `G20=algebra([1,1],["e1","e2"])`, `e1=G20[1]`, `simplify(-9.81*M*cos(x)*e1*kg+S*cos(x)**2+S*sin(x)**2)`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine();
        const { trees, errors } = engine.parse(sourceText, {});
        assert.strictEqual(errors.length, 0);
