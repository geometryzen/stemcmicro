...
describe("tan", function () {
    ...
    it("f(360,0)", function () {
        const lines: string[] = [`f(a,x)=1+tan(float(a/360*2*pi))-float(x)+tan(a/360*2*pi)-x`, `f(360,0)`];
        const engine = create_script_context({
            dependencies: ["Flt"],
            prolog: [`pi=tau(1/2)`]
        });
    ...
    it("f(135,-1)", function () {
        const lines: string[] = [`f(a,x)=1+tan(float(a/360*2*pi))-float(x)+tan(a/360*2*pi)-x`, `f(135,-1)`];
        const engine = create_script_context({
            dependencies: ["Flt"],
            prolog: [`pi=tau(1/2)`]
        });
