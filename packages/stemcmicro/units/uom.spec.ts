...
describe("uom", function () {
    it("GA and uom", function () {
        const lines: string[] = [`G30=algebra([1,1,1],["e1","e2","e3"])`, `e1=G30[1]`, `e2=G30[2]`, `m=uom("meter")`, `s=uom("second")`, `g=9.81*(-e2) * m/s/s`, `g`];
    ...
    it("Handling middot", function () {
        const lines: string[] = [`k=uom("kilogram")`, `m=uom("meter")`, `s=uom("second")`, `k * m / s`];
        const sourceText = lines.join("\n");
        const engine: ExprEngine = create_engine();
        const { trees, errors } = engine.parse(sourceText);
        assert.strictEqual(errors.length, 0);
...
describe("SI units", function () {
    it("/(Uom, Flt)", function () {
        const lines: string[] = [`kg = uom("kilogram")`, `kg / 5.0`];
...
describe("uom", function () {
    describe("SI units", function () {
        it("kilogram", function () {
            const lines: string[] = [`uom("kilogram")`];
            const engine = create_script_context({
                dependencies: ["Uom"]
            });
            const actual = assert_one_value_execute(lines.join("\n"), engine);
        ...
        it("coulomb", function () {
            const lines: string[] = [`uom("coulomb")`];
            const engine = create_script_context({
                dependencies: ["Uom"]
            });
            const actual = assert_one_value_execute(lines.join("\n"), engine);
        ...
        it("candela", function () {
            const lines: string[] = [`uom("candela")`];
            const engine = create_script_context({
                dependencies: ["Uom"]
            });
            const actual = assert_one_value_execute(lines.join("\n"), engine);
    ...
    describe("operator *", function () {
        it("(Uom, Uom)", function () {
            const lines: string[] = [`kg = uom("kilogram")`, `m = uom("meter")`, `kg * m`];
            const engine = create_script_context({
                dependencies: ["Uom"]
            });
            const actual = assert_one_value_execute(lines.join("\n"), engine);
        ...
        it("(Flt, Uom)", function () {
            const lines: string[] = [`kg = uom("kilogram")`, `5.0 * kg`];
            const engine = create_script_context({
                dependencies: ["Flt", "Uom"]
            });
            const actual = assert_one_value_execute(lines.join("\n"), engine);
        ...
        it("(Num, Uom)", function () {
            const lines: string[] = [`kg = uom("kilogram")`, `2 * kg`];
            const engine = create_script_context({
                dependencies: ["Uom"]
            });
            const actual = assert_one_value_execute(lines.join("\n"), engine);
        ...
        it("(Uom, Num)", function () {
            const lines: string[] = [`kg = uom("kilogram")`, `kg * 2`];
            const engine = create_script_context({
                dependencies: ["Uom"]
            });
            const actual = assert_one_value_execute(lines.join("\n"), engine);
        ...
        it("(Vec, Uom)", function () {
            const lines: string[] = [`G = algebra([1],["e1"])`, `e1=G[1]`, `kg = uom("kilogram")`, `e1 * kg`];
            const engine = create_script_context({
        ...
        it("(Uom, Vec)", function () {
            const lines: string[] = [`G = algebra([1],["e1"])`, `e1=G[1]`, `kg = uom("kilogram")`, `kg * e1`];
            const engine = create_script_context({
                dependencies: ["Blade", "Uom"]
            });
            const actual = assert_one_value_execute(lines.join("\n"), engine);
    ...
    describe("operator /", function () {
        it("(Uom, Uom)", function () {
            const lines: string[] = [`m = uom("meter")`, `s = uom("second")`, `m / s`];
            const engine = create_script_context({
                dependencies: ["Uom"]
            });
            const actual = assert_one_value_execute(lines.join("\n"), engine);
    ...
    describe("operator +", function () {
        it("(Uom, Uom)", function () {
            const lines: string[] = [`m = uom("meter")`, `m + m`];
            const engine = create_script_context({
                dependencies: ["Uom"]
            });
            const actual = assert_one_value_execute(lines.join("\n"), engine);
        ...
        it("(Flt, Uom)", function () {
            const lines: string[] = [`kg = uom("kilogram")`, `6.0 + kg`];
            const engine = create_script_context({
    ...
    describe("derived", function () {
        it("kg * m / s / s => N", function () {
            const lines: string[] = [`kg = uom("kilogram")`, `m = uom("meter")`, `s = uom("second")`, `kg * m / s / s`];
            const engine = create_script_context({
                dependencies: ["Uom"]
            });
            const actual = assert_one_value_execute(lines.join("\n"), engine);
        ...
        it("kg * m / s ** 2 => N", function () {
            const lines: string[] = [`kg = uom("kilogram")`, `m = uom("meter")`, `s = uom("second")`, `kg * m / s ** 2`];
            const engine = create_script_context({
                dependencies: ["Uom"]
            });
            const actual = assert_one_value_execute(lines.join("\n"), engine);
        ...
        it("J / C => V", function () {
            const lines: string[] = [`kg = uom("kilogram")`, `m = uom("meter")`, `s = uom("second")`, `C = uom("coulomb")`, `N = kg * m / s / s`, `J = N * m`, `J / C`];
            const engine = create_script_context({
                dependencies: ["Uom"]
            });
            const actual = assert_one_value_execute(lines.join("\n"), engine);
        ...
        it("A * s => C", function () {
            const lines: string[] = [`A = uom("ampere")`, `s = uom("second")`, `A * s`];
            const engine = create_script_context({
                dependencies: ["Uom"]
            });
    ...
    describe("functions", function () {
        it("abs(uom(kg))", function () {
            const lines: string[] = [`kg = uom("kilogram")`, `abs(kg)`];
            const engine = create_script_context({
                dependencies: ["Flt", "Uom"]
            });
            const actual = assert_one_value_execute(lines.join("\n"), engine);
    ...
    describe("dimensionless", function () {
        it("second", function () {
            const lines: string[] = [`second=uom("second")`, `second`];
            const sourceText = lines.join("\n");
            const engine: ExprEngine = create_engine();
            const { trees, errors } = engine.parse(sourceText, {});
            assert.strictEqual(errors.length, 0);
        ...
        it("dimensionless Uom", function () {
            const lines: string[] = [`second=uom("second")`, `second/second`];
            const sourceText = lines.join("\n");
            const engine: ExprEngine = create_engine();
            const { trees, errors } = engine.parse(sourceText, {});
            assert.strictEqual(errors.length, 0);
