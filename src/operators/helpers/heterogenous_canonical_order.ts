...
class Builder<L extends U, R extends U> implements OperatorBuilder<Cons2<Sym, L, R>> {
    ...
    create($: ExtensionEnv): Operator<Cons2<Sym, L, R>> {
        return new FlipOperator(this.name, this.hash, this.opr, this.guardL, this.guardR, $);
    }
}

class FlipOperator<L extends U, R extends U> extends Function2<L, R> implements Operator<Cons2<Sym, L, R>> {
