...
export function number_to_floating_point_string(d: number, $: ExtensionEnv): string {
    ...
    if ($.iszero($.getSymbolBinding(FORCE_FIXED_PRINTOUT))) {
        str = '' + d;
        ...
        const txtMaxFixedPrintoutDigits = $.getSymbolBinding(VARNAME_MAX_FIXED_PRINTOUT_DIGITS);
...
function shouldForceFixed($: ExtensionEnv): boolean {
    const forceFixedBinding = $.getSymbolBinding(FORCE_FIXED_PRINTOUT);
