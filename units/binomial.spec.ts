...
describe("binomial", function () {
    ...
    it("binomial(0,k)", function () {
        ...
        const { values } = engine.executeScript(lines.join('\n'));
        assert.strictEqual(engine.renderAsSExpr(values[0]), "(* (power (factorial k) -1) (power (factorial (* -1 k)) -1))");
    ...
    xit("binomial(n,0)", function () {
        ...
        const { values } = engine.executeScript(lines.join('\n'));
        assert.strictEqual(engine.renderAsSExpr(values[0]), "(* (factorial n) (power (factorial n) -1))");
