...
import { InputState } from "../src/scanner/InputState";
import { is_digit } from "../src/scanner/is_digit";
...
const num = alt(function (node: unknown) {
...
const exp = alt(function (node: unknown) {
    if (Array.isArray(node)) {
...
const factor = alt(function (node) {
    return node;
}, ref('num'), ref('grouping'));
